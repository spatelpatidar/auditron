...
RSpec.describe Auditron::Generators::InstallGenerator do
  ...
  # ---------------------------------------------------------------------------
  # #print_logo (was display_banner)
  # ---------------------------------------------------------------------------

  describe "#print_logo" do
    it "outputs the gem name" do
      gen = build_generator
      expect($stdout).to receive(:print).with(/Audit Logging for Rails/i).at_least(:once)
    ...
    it "outputs the current VERSION" do
      gen = build_generator
      expect($stdout).to receive(:print).with(/#{Regexp.escape(Auditron::VERSION)}/).at_least(:once)
      allow($stdout).to receive(:print)
      allow($stdout).to receive(:flush)
      allow($stdout).to receive(:puts)
      gen.send(:print_logo)
  ...
  # ---------------------------------------------------------------------------
  # #print_intro (was display_intro)
  # ---------------------------------------------------------------------------

  describe "#print_intro" do
    it "mentions audit_logs migration" do
      gen = build_generator
      expect($stdout).to receive(:puts).with(/audit_logs/).at_least(:once)
      allow($stdout).to receive(:puts)
      allow($stdout).to receive(:flush)
      gen.send(:print_intro)
    end

    it "mentions the initializer" do
      gen = build_generator
      expect($stdout).to receive(:puts).with(/initializers\/auditron\.rb|auditron\.rb/).at_least(:once)
      allow($stdout).to receive(:puts)
      allow($stdout).to receive(:flush)
      gen.send(:print_intro)
    end
  ...
  # ---------------------------------------------------------------------------
  # #print_next_steps (was display_initializer_hint)
  # ---------------------------------------------------------------------------

  describe "#print_next_steps" do
    before do
      # print_next_steps references @cfg[:actor_method], so set it up
      subject.instance_variable_set(:@cfg, { actor_method: "current_user" })
    end

    let(:subject) { build_generator }

    it "tells the user to run rails db:migrate" do
      expect($stdout).to receive(:puts).with(/rails db:migrate/).at_least(:once)
      allow($stdout).to receive(:puts)
      allow($stdout).to receive(:flush)
      subject.send(:print_next_steps)
    end

    it "shows the Auditron.configure block" do
      expect($stdout).to receive(:puts).with(/Auditron/).at_least(:once)
      allow($stdout).to receive(:puts)
      allow($stdout).to receive(:flush)
      subject.send(:print_next_steps)
    end
    ...
    it "shows set_audit_actor controller example" do
      expect($stdout).to receive(:puts).with(/set_audit_actor/).at_least(:once)
      allow($stdout).to receive(:puts)
      allow($stdout).to receive(:flush)
      subject.send(:print_next_steps)
    end
  ...
  # ---------------------------------------------------------------------------
  # #print_done (was display_success)
  # ---------------------------------------------------------------------------

  describe "#print_done" do
    it "says installed successfully" do
      gen = build_generator
      expect($stdout).to receive(:puts).with(/installed successfully/).at_least(:once)
      allow($stdout).to receive(:puts)
      allow($stdout).to receive(:flush)
      gen.send(:print_done)
    end
  ...
  describe "#cancel_install" do
    it "outputs a cancellation message" do
      gen = build_generator
      expect($stdout).to receive(:puts).with(/CANCELLED|cancelled/i).at_least(:once)
      allow($stdout).to receive(:puts)
      allow($stdout).to receive(:flush)
    ...
    it "tells user how to re-run the generator" do
      gen = build_generator
      expect($stdout).to receive(:puts).with(/rails generate auditron:install/).at_least(:once)
      allow($stdout).to receive(:puts)
      allow($stdout).to receive(:flush)
      gen.send(:cancel_install)
    end
  ...
  describe "#install when user confirms" do
    def stub_install_inputs(gen, confirm: "y")
      # confirm, ignored_fields, store_ip, retention_days, actor_method
      allow($stdin).to receive(:gets).and_return(
        "#{confirm}\n", "\n", "y\n", "\n", "\n"
      )
      allow($stdout).to receive(:puts)
    ...
    it "calls migration_template with the correct arguments" do
      gen = build_generator
      stub_install_inputs(gen, confirm: "y")
    ...
    it "calls print_next_steps and print_done" do
      gen = build_generator
      stub_install_inputs(gen, confirm: "y")
      expect(gen).to receive(:print_next_steps).and_call_original
      expect(gen).to receive(:print_done).and_call_original
      gen.install
    end

    it "does NOT call cancel_install" do
      gen = build_generator
      stub_install_inputs(gen, confirm: "yes")
  ...
  describe "#install when user declines" do
    ...
    it "does NOT call print_done" do
      gen = build_generator
      stub_declined_inputs(gen)
      expect(gen).not_to receive(:print_done)
      gen.install
    end
  end
end
